...
def altaz_midpoint(a1, a2):
    c1 = a1.represent_as(CartesianRepresentation)
    c2 = a2.represent_as(CartesianRepresentation)
    return AltAz((c1 + c2) / 2)
...
OI_MAPPING: dict[str, ColumnMapping] = {
    ...
    "s_ra": (lambda coo: coo.ra.deg, "tracking_radec"),
    "s_dec": (lambda coo: coo.dec.deg, "tracking_radec"),
    ...
    "azimuth": (
        lambda a1, a2: altaz_midpoint(a1, a2).az.deg,
        "altaz_begin",
        "altaz_end",
    ),
    ...
    "altitude": (
        lambda a1, a2: altaz_midpoint(a1, a2).alt.deg,
        "altaz_begin",
        "altaz_end",
    ),
    ...
    "zenith_distance": (
        lambda a1, a2: altaz_midpoint(a1, a2).zen.deg,
        "altaz_begin",
        "altaz_end",
    ),
...
def process_oi_column(column_def: ColumnMapping, obs_info: ObservationInfo) -> Any:
    ...
    if isinstance(column_def, str):
        if hasattr(obs_info, column_def):
            return getattr(obs_info, column_def)
    elif isinstance(column_def, tuple):
        fn = column_def[0]
...
def process_resource(resource: ResourcePath, instrument_dict: dict, update: bool = False) -> None:
    ...
    for column, column_def in OI_MAPPING.items():
        try:
            ...
            if isinstance(exposure_rec[column], Quantity):
            ...
            logger.error(f"Processed {column:>15}: {exposure_rec[column]!r}")
        except Exception:
