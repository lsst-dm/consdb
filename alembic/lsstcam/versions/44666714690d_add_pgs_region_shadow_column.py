"""Add pgs_region shadow column

Revision ID: 44666714690d
Revises: 7058c1175b5c
Create Date: 2025-09-30 19:09:02.809399+00:00

"""

from collections.abc import Sequence

import sqlalchemy as sa
from alembic import op
from lsst.consdb.cdb_pgsphere import SPoly

# revision identifiers, used by Alembic.
revision: str = "44666714690d"
down_revision: str | None = "7058c1175b5c"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None


def upgrade() -> None:
    op.execute("DROP VIEW IF EXISTS cdb_lsstcam.ccdvisit1")
    op.execute("DROP VIEW IF EXISTS cdb_lsstcam.visit1")
    op.add_column(
        "ccdexposure",
        sa.Column(
            "pgs_region",
            SPoly(),
            nullable=True,
            comment="Spherical region type spoly corresponding to s_region.",
        ),
        schema="cdb_lsstcam",
    )
    op.add_column(
        "exposure",
        sa.Column(
            "pgs_region",
            SPoly(),
            nullable=True,
            comment="Spherical region type spoly corresponding to s_region.",
        ),
        schema="cdb_lsstcam",
    )
    op.execute(
        "CREATE OR REPLACE VIEW cdb_lsstcam.visit1 AS\n        SELECT exposure_id AS visit_id, exposure_name, controller, day_obs, seq_num, physical_filter, band, s_ra, s_dec, sky_rotation, azimuth_start, azimuth_end, azimuth, altitude_start, altitude_end, altitude, zenith_distance_start, zenith_distance_end, zenith_distance, airmass, exp_midpt, exp_midpt_mjd, obs_start, obs_start_mjd, obs_end, obs_end_mjd, exp_time, shut_time, dark_time, group_id, cur_index, max_index, img_type, emulated, science_program, observation_reason, target_name, air_temp, pressure, humidity, wind_speed, wind_dir, dimm_seeing, focus_z, simulated, vignette, vignette_min, scheduler_note, s_region, can_see_sky, pgs_region\n        FROM cdb_lsstcam.exposure;"
    )
    op.execute("GRANT SELECT ON cdb_lsstcam.visit1 TO usdf;")
    op.execute("GRANT SELECT ON cdb_lsstcam.visit1 TO oods;")
    op.execute(
        "CREATE OR REPLACE VIEW cdb_lsstcam.ccdvisit1 AS\n        SELECT ccdexposure_id AS ccdvisit_id, exposure_id AS visit_id, day_obs, seq_num, detector, s_region, pgs_region\n        FROM cdb_lsstcam.ccdexposure;"
    )
    op.execute("GRANT SELECT ON cdb_lsstcam.ccdvisit1 TO usdf;")
    op.execute("GRANT SELECT ON cdb_lsstcam.ccdvisit1 TO oods;")


def downgrade() -> None:
    op.execute("DROP VIEW IF EXISTS cdb_lsstcam.ccdvisit1")
    op.execute("DROP VIEW IF EXISTS cdb_lsstcam.visit1")
    op.drop_column("exposure", "pgs_region", schema="cdb_lsstcam")
    op.drop_column("ccdexposure", "pgs_region", schema="cdb_lsstcam")
    op.execute("CREATE VIEW cdb_lsstcam.ccdvisit1 AS SELECT * FROM cdb_lsstcam.ccdexposure")
    op.execute("CREATE VIEW cdb_lsstcam.visit1 AS SELECT * FROM cdb_lsstcam.exposure")
    op.execute("ALTER TABLE cdb_lsstcam.ccdvisit1 RENAME COLUMN ccdexposure_id TO ccdvisit_id")
    op.execute("ALTER TABLE cdb_lsstcam.ccdvisit1 RENAME COLUMN exposure_id TO visit_id")
    op.execute("ALTER TABLE cdb_lsstcam.visit1 RENAME COLUMN exposure_id TO visit_id")
    op.execute("GRANT SELECT ON cdb_lsstcam.ccdvisit1 TO usdf")
    op.execute("GRANT SELECT ON cdb_lsstcam.ccdvisit1 TO oods")
    op.execute("GRANT SELECT ON cdb_lsstcam.visit1 TO usdf")
    op.execute("GRANT SELECT ON cdb_lsstcam.visit1 TO oods")
