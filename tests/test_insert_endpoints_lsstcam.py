...
def lsstcam_client():
    ...
    schema_file = Path(__file__).parents[1] / "sdm_schemas" / "yml" / f"{schema_name}.yaml"
...
def _call_insert_multiple(client: TestClient, table_name: str, table: sa.Table, row: dict[str, object], u: int):
    obs_id_col = _obs_id_column(table)
